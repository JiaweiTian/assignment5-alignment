...
from zero_shot_baseline import (load_cot_prompt,
                                look_up_dataset,
                                evaluate_vllm
                                )
...
class SFTDataset(Dataset):
    def __init__(self, inputs_data, num_samples, tokenizer):
        ...
        for i in random_idx:
            prompt_strs.append(load_cot_prompt(inputs_data[i]['question']))
            output_strs.append(inputs_data[i]['answer'])

        self.dictionary = tokenize_prompt_and_output(prompt_strs, output_strs, tokenizer)
...
def main(args):
    ...
    valid_prompts = [load_cot_prompt(valid_data[i]['question']) for i in range(len(valid_data))]
    valid_answers = [valid_data[i]['answer'] for i in range(len(valid_data))]
    # valid_input_ids, valid_labels, valid_response_mask = tokenize_prompt_and_output(valid_prompts, valid_answers, tokenizer)
    ...
    vllm_model = init_vllm(model_name, device, SEED, gpu_memory_utilization=0.85)
    ...
    for epoch in range(args.num_epochs):
        ...
        with torch.no_grad():
            ...
            results = evaluate_vllm(vllm_model, r1_zero_reward_fn, valid_prompts, valid_answers, sampling_params, False)
...
def get_training_args():
    ...
    parser.add_argument('--num_epochs', type=int, default=5, help='total number of training epoch')
    parser.add_argument('--num_samples', type=int, default=128, help='number of unique examples for SFT')
    parser.add_argument('--warmup_ratio', type=float, default=0.1, help='ratios for warmup steps')
    parser.add_argument('--batch_size', type=int, default=4, help='batch_size during training')
